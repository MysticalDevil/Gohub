...
- 项目当前目标 Go 版本为 `go 1.26`。
- API 响应已统一为 `code/msg/data/errors` 信封结构。
- 分页使用 `offset/limit/sort/order`，排序字段由源码白名单约束。
- 迁移回调返回 `error`；迁移失败不会写入成功记录。
- 模型写操作向控制器返回数据库错误，不再只依赖 `RowsAffected`。
- 真实 HTTP smoke 测试入口为 `scripts/api_smoke_test.py`。
...
2. `go test ./...` 通过。
3. `go vet ./...` 通过。
4. `python3 scripts/api_smoke_test.py --base-url http://127.0.0.1:3000`
   覆盖全部当前 API 路由。
5. 行为一致、API 语义统一、可回滚。
